						...
						<div class="col-xs-12">
							<div class="topDangNhap" style="float: right;">
								<div id="topuser" class="textWhite">
									...
									<a id="user" class="aDangNhap" href="#"
										data-toggle="modal" data-target="#formlogin"><i class="fa fa-user"></i> Đăng nhập </a>
									<%}else{ %>
									<%User user = (User) session.getAttribute("user");
											 	%>
									<script type="text/javascript">
											 	$("#topuser").html("<div class='dropdown'> <a id='user' class='aDangNhap dropdown-toggle' href='#' data-toggle='dropdown'> <i class='fa fa-user'> </i> Xin chào <%=user.getName()%></a>"
											 			+"<ul class='dropdown-menu'> <li><a href='#'>Quản lý đơn hàng</a></li> <li><a href='Profile.jsp'>Thông tin</a></li>   <li><a href='<%=request.getContextPath()%>/Logout''>Thoát</a></li>   </ul> </div>");
											 </script>
									<%} %>
								</div>
							</div>
							<% TreeMap<Integer,Integer> map = null;
								 if (session.getAttribute("cart") == null ){ map = new TreeMap<Integer, Integer>();}
								 else{
								 map =  (TreeMap<Integer, Integer>) session.getAttribute("cart");}
								 request.getSession().setAttribute("cart", map);
								 request.setAttribute("cart", map); 
								 %>
							<div class="topDangNhap" style="float: right;">
								<div class="textWhite">
									<a class="gioHang" href="cart.jsp">Giỏ hàng (<b
										id="qualityCart"><%= map.size() %></b> sản phẩm)
									</a>
								</div>
							</div>
						</div>
				...
				<div class="modal-body">
					<div class="tab-content">
						<div id="login-form" class="tab-pane fade in active">
							<div id="err_login" class="required"></div>
						...
						<div id="registration-form" class="tab-pane fade">
							<form id="myForm"
								action="<%=request.getContextPath()%>/Checkregister"
								method="post">
								<div class="form-group">
									<div class="form-group">
										<label for="tendangnhap">Tên đăng nhập:</label> <input
											type="text" class="form-control"
											placeholder="Enter account login" name="account">
									</div>
									<div class="form-group">
									...
									<div class="form-group">
										<label for="passwordAgain">Nhập lại Mật khẩu:</label> <input
											...
											placeholder="New password" onkeyup="checkpass()"
											name="passwordAgain"> <span id="errComfim"
											class="requiment"></span>
									</div>
								...
								<div class="form-group">
									<label for="ngaysinh">Ngày Sinh:</label> <input type="date"
										class="form-control" value="2018-07-22" min="1921-01-01"
										max="2018-12-14" name="ngaysinh">
								</div>
								<div class="form-group">
		...
		$.ajax({
			...
			success: function(data) {
				...
				if (data.length >60) {
					...
					$("#topuser").html("<div class='dropdown'> <a id='user' class='aDangNhap dropdown-toggle' href='#' data-toggle='dropdown'> <i class='fa fa-user'> </i> Xin chào" +obj.name +"</a>"
				 			+"<ul class='dropdown-menu'> <li><a href='#'>Quản lý đơn hàng</a></li> <li><a href='Profile.jsp'>Thông tin</a></li>   <li><a href='<%=request.getContextPath()%>/Logout''>Thoát</a></li>   </ul> </div>");
						} else {
							$("#err_login").text(data);
						}
...
<script type="text/javascript">
	function checkpass() {
		if ($("#password").val() == $("#passwordAgain").val()) {
			$("#errComfim").html(
					"<i class='fa fa-check-circle' style = 'color :#00e900;'></i>");
		} else {
			$("#errComfim").html("<span style='color:red;'> Mật khẩu không khớp!<span>");
